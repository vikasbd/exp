...
void thread_loop(void *userdata) {
	...
	int fd = net_connect_tcp(state->target_addr, state->src_port);
...
int main(int argc, const char *argv[])
{
	...
	const char *payload = (const char[8192]){0};
	int pktsize = atoi(getenv("PACKET_SIZE"));
	int payload_sz = pktsize - 42;
